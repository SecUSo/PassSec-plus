var changes = [];
var defaultRedirects = ["google.de","de-de.facebook.com","youtube.com", "google.com", "de.wikipedia.org", "wikipedia.de", "de.yahoo.com", "login.yahoo.com", "tumblr.com"];
...
function fillRedirectList(){
  ...
  for(i=0; i<redirects.length;i++){
    ...
    $("#redirects"+i).on("click",function(e){
      ...
      var index = $(this).attr("id").replace("redirects","");
...
function fillExceptionList(){
  ...
  var table = document.getElementById("exceptionList");
  table.getElementsByTagName("tbody")[0].innerHTML = table.rows[0].innerHTML;
  ...
  for(i=0; i<exceptions.length;i++){
    ...
    $("#exceptions"+i).on("click",function(e){
      save("exceptions", window.localStorage.getItem("exceptions"));
      var element = $(this).next().html();
      var index = $(this).attr("id").replace("exceptions","");
      $(this).parent().parent().parent().remove();
      var arr = [];
      try{
